...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    def __init__(
        ...
        """
        self.api_key = api_key or settings.djgent_settings.get(
            "EMBEDDING_API_KEY"
        )
        if not self.api_key:
            raise ValueError(
                "Embedding API key is required. Set EMBEDDING_API_KEY in settings or .env file."
            )
...
class HuggingFaceEmbeddingProvider(EmbeddingProvider):
    ...
    def get_default_model(self) -> str:
        ...
        return settings.djgent_settings.get(
